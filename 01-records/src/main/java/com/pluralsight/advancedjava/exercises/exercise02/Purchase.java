...
public record Purchase(long productId, int quantity, BigDecimal price, LocalDateTime dateTime) {
    public Purchase {
        Validation.checkThis(quantity>0,"quantity must be greater than zero");
        Validation.checkThis(price.compareTo(BigDecimal.ZERO)>0,"price must be greater than zero");
        Validation.checkThis(dateTime.isBefore(LocalDateTime.now()),"dateTime must be in the past (not in the future).");
    }
}
